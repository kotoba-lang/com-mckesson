(ns mckesson.main
  "Kotodama WASM entrypoint for the Mckesson clean-room actor (L4) — Clojure port.
  ...
  facts (`mckesson.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mckesson")
(def tier "L4")
...
(def entity-specs
  [{:entity "Prescription" :plural "prescriptions" :id-prefix "mckesson_pre"
   ...
   {:entity "Claim" :plural "claims" :id-prefix "mckesson_cla"
   ...
   {:entity "Pharmacy" :plural "pharmacies" :id-prefix "mckesson_pha"
    :fields [:npi :name :network] :required [:npi :name]
    :coerce {} :refs {}}
   {:entity "Drug" :plural "drugs" :id-prefix "mckesson_dru"
    :fields [:ndc :name :strength :form] :required [:ndc :name]
    :coerce {} :refs {}}
   {:entity "Member" :plural "members" :id-prefix "mckesson_mem"
    :fields [:memberId :planId :groupId] :required []
    :coerce {} :refs {}}
   {:entity "PriorAuth" :plural "priorauths" :id-prefix "mckesson_pri"
...
(defn emit-facts
  "EAVT facts for one record: {\"mckesson.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mckesson-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mckesson-compat/ready)
